...
**주문 API** (30개 테스트) - 최소 단위로 세분화
- **POST /api/orders** (14개 테스트)
  - ⏭️ TC-O1.1: HTTP 상태 코드 검증 (더미 모드 스킵)
  - ⏭️ TC-O1.2: 응답 본문 구조 검증 (더미 모드 스킵)
  - ✅ TC-O2.1: user_id 누락 시 400 에러
  - ✅ TC-O2.2: items 누락 시 400 에러
  - ✅ TC-O2.3: 빈 items 배열 시 400 에러
  - ✅ TC-O2.4: user_id와 items 모두 누락 시 400 에러
  - ✅ TC-O3.1: user_id 타입 검증
  - ✅ TC-O3.2: items 배열 구조 검증
  - ✅ TC-O3.3: menu_id 필수 필드 검증
  - ✅ TC-O3.4: options 필드 처리 검증
  - ⏭️ TC-O4.1: order_id 타입 및 범위 검증 (더미 모드 스킵)
  - ⏭️ TC-O4.2: status 기본값 검증 (더미 모드 스킵)
  - ⏭️ TC-O4.3: created_at 타입 검증 (더미 모드 스킵)
  - ✅ TC-O5.1: 서버 에러 처리
- **GET /api/orders** (16개 테스트)
...
| 날짜 | 테스트 수 | 통과 | 실패 | 커버리지 | 비고 |
|------|-----------|------|------|----------|------|
| 2025-11-01 (초기) | 54 | 54 | 0 | 72.52% | - |
| 2025-11-01 (업데이트) | 87 | 87 | 0 | 72.52% | 메뉴 API (+10개), 주문 API (+23개) 최소 단위 테스트 추가 |
